<?php
class dm_Widget_Twitter extends WP_Widget {

	function dm_Widget_Twitter() {
		$widget_ops = array('classname' => 'dm_featured_works', 'description' => __( "Display your latest tweets.") );
		$this->WP_Widget('dm-twitter', __('TouchSense Twitter Widget'), $widget_ops);
		$this->alt_option_name = 'dm_widget_twitter';
	...
	function widget($args, $instance) {
		$cache = wp_cache_get('dm_Widget_Twitter ', 'widget');
...
?>
		...
		<ul id="twitter_update_listdm"><li>Twitter feed loading...</li></ul>
		<?php echo $after_widget; ?>
<?php
$unamed = $instance['tw_username'];
$nr = $instance['number'];
echo '<script type="text/javascript" src="'. get_template_directory_uri() .'/scripts/twitter-rss-with-rt.js"></script>';
echo "<script type='text/javascript'>
         jQuery(document).ready(function() {
            GetTwitterFeedIncRT('".$unamed."', 3, 'twitter_update_listdm', 1);
	 });
      </script>"
?>
...
<?php
		...
		wp_cache_set('dm_widget_twitter', $cache, 'widget');
	}
	...
	function flush_widget_cache() {
		wp_cache_delete('dm_widget_twitter', 'widget');
	}
...
add_action('widgets_init', create_function('', 'return register_widget("dm_Widget_Twitter");'));
?>